...
function renderSurface(seed, surfaceSlant, choice, light, lightSlant,
  material, amplitude, isPretest, ambientLightIntensity) {
...
function findAllAmbientLightIntensity() {
  // findAmbientLightIntensity(surfaceSlant = 30, material = MATERIALS.MATTE, lo = 0, hi = 1, target = TARGET_RMS.MATTE[30]);
  // findAmbientLightIntensity(surfaceSlant = 45, material = MATERIALS.MATTE, lo = 0, hi = 1, target = TARGET_RMS.MATTE[45]);
  // findAmbientLightIntensity(surfaceSlant = 60, material = MATERIALS.MATTE, lo = 0, hi = 1, target = TARGET_RMS.MATTE[60]);

  // findAmbientLightIntensity(surfaceSlant = 30, material = MATERIALS.GLOSSY, lo = 0, hi = 1, target = TARGET_RMS.GLOSSY[30]);
  // findAmbientLightIntensity(surfaceSlant = 45, material = MATERIALS.GLOSSY, lo = 0, hi = 1, target = TARGET_RMS.GLOSSY[45]);
  // findAmbientLightIntensity(surfaceSlant = 60, material = MATERIALS.GLOSSY, lo = 0, hi = 1, target = TARGET_RMS.GLOSSY[60]);
}
        ...
        for (let materialIndex = 0; materialIndex < materials.length; materialIndex += 1) {
          ...
          for (let lightSlantIndex = 0;
            ...
            if (data.material == MATERIALS.MATTE) {
              ambientLightIntensity = LIGHT_INTENSITY_MATTE[data.surfaceSlant][data.lightSlant];
            } else {
              ambientLightIntensity = LIGHT_INTENSITY_GLOSSY[data.surfaceSlant][data.lightSlant];
            }

            const amplitude = AMPLITUDES[data.surfaceSlant];
            const tempCanvas = renderSurface(data.seed, data.surfaceSlant, data.choice,
            data.light, data.lightSlant, data.material, amplitude, data.isPretest, ambientLightIntensity);
            const filename = getSurfaceInfoString(data, '.png');
            tempCanvas.then((canvas) => {
              downloadCanvas(canvas, filename);
            });
