...
const CUSTOMFRAGMENTSHADERCONTRAST = [
  '#define PHONG',
  ...
  ' vec4 v = vec4( outgoingLight, diffuseColor.a ), gammafactor);',
  ' vec4 normalized = normalizeContrast( meanTarget, meanIntensity, stdTarget, stdIntensity, v);',
  ' gl_FragColor = LinearToGamma(normalized);',
...
function CustomContrastUniform(gammaFactor, meanTarget, meanIntensity, stdTarget, stdIntensity) {
  const uniforms = THREE.UniformsUtils.merge([
    THREE.ShaderLib.phong.uniforms,
    {
      meanTarget: { value: meanTarget },
      meanIntensity: { value: meanIntensity },
      stdTarget: { value: stdTarget },
      stdIntensity: { value: stdIntensity },
      gammafactor: { value: gammaFactor },
    },
  ]);
...
function CustomContrastMatteMaterial(uniform) {
  const matteMaterial = new THREE.ShaderMaterial({
    uniforms: uniform,
    vertexShader: THREE.ShaderLib.phong.vertexShader,
...
function CustomContrastGlossyMaterial(uniform) {
  const glossyMaterial = new THREE.ShaderMaterial({
    uniforms: uniform,
    vertexShader: THREE.ShaderLib.phong.vertexShader,
    fragmentShader: CUSTOMFRAGMENTSHADERCONTRAST,
...
function getAllContrastMaterial(gammaFactor) {
  const contrastNormalizedMaterials = {};
  contrastNormalizedMaterials[MATERIALS.MATTE] = {};
  contrastNormalizedMaterials[MATERIALS.GLOSSY] = {};
  contrastNormalizedMaterials[MATERIALS.MATTE][30] = {};
  contrastNormalizedMaterials[MATERIALS.MATTE][45] = {};
  contrastNormalizedMaterials[MATERIALS.MATTE][60] = {};
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30] = {};
  contrastNormalizedMaterials[MATERIALS.GLOSSY][45] = {};
  contrastNormalizedMaterials[MATERIALS.GLOSSY][60] = {};

  const targetStd = 10.378200810233588;
  const meanTarget = 164.3208475484253;

  contrastNormalizedMaterials[MATERIALS.MATTE][30][20] = CustomContrastUniform(gammaFactor,
    meanTarget, 156.78441751556545, targetStd, 15.378369292846338); // matte directional 20 30
  contrastNormalizedMaterials[MATERIALS.MATTE][30][30] = CustomContrastUniform(gammaFactor,
    meanTarget, 156.73204315863583, targetStd, 14.735199406339916); // matte directional 30 30
  contrastNormalizedMaterials[MATERIALS.MATTE][30][40] = CustomContrastUniform(gammaFactor,
    meanTarget, 152.43828727743295, targetStd, 19.189763292808713); // matte directional 40 30
  contrastNormalizedMaterials[MATERIALS.MATTE][30][50] = CustomContrastUniform(gammaFactor,
    meanTarget, 141.85212866714215, targetStd, 26.63428501073127); // matte directional 50 30
  contrastNormalizedMaterials[MATERIALS.MATTE][30][60] = CustomContrastUniform(gammaFactor,
    meanTarget, 128.97426185358717, targetStd, 33.95757496043813); // matte directional 60 30
  contrastNormalizedMaterials[MATERIALS.MATTE][30][70] = CustomContrastUniform(gammaFactor,
    meanTarget, 110.45247650436139, targetStd, 40.618685834439965); // matte directional 70 30

  contrastNormalizedMaterials[MATERIALS.MATTE][30][LIGHTS.MATLAB] = CustomContrastUniform(
    gammaFactor, meanTarget, 145.24504151641847, targetStd, 68.19507984420792); // matte matlab 30

  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][20] = CustomContrastUniform(gammaFactor,
    meanTarget, 77.66105048482835, targetStd, 20.840488892152457); // glossy directional 20 30
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][30] = CustomContrastUniform(gammaFactor,
    meanTarget, 77.59462339901249, targetStd, 19.640071841171896); // glossy directiona 30 30
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][40] = CustomContrastUniform(gammaFactor,
    meanTarget, 75.66843347453761, targetStd, 20.51199028233263); // glossy directional 40 30
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][50] = CustomContrastUniform(gammaFactor,
    meanTarget, 70.6609089975541, targetStd, 21.623171581600857); // glossy directional 50 30
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][60] = CustomContrastUniform(gammaFactor,
    meanTarget, 63.91771310005013, targetStd, 23.762505138501464); // glossy directional 60 30
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][70] = CustomContrastUniform(gammaFactor,
    meanTarget, 54.97124607349393, targetStd, 25.795451080781323); // glossy directional 70 30
  
  contrastNormalizedMaterials[MATERIALS.GLOSSY][30][LIGHTS.MATLAB] = CustomContrastUniform(
    gammaFactor, meanTarget, 130.73872104507092, targetStd, 59.37215513448607); // matlab glossy 30

  contrastNormalizedMaterials[MATERIALS.MATTE][45][30] = CustomContrastUniform(gammaFactor,
    meanTarget, 161.82561866291567, targetStd, 13.962054755936498); // matte directional 30 45
  contrastNormalizedMaterials[MATERIALS.MATTE][45][45] = CustomContrastUniform(gammaFactor,
    meanTarget, 164.3208475484253, targetStd, 10.378200810233588); // matte directional 45 45
  contrastNormalizedMaterials[MATERIALS.MATTE][45][60] = CustomContrastUniform(gammaFactor,
    meanTarget, 154.0722337735319, targetStd, 17.928364448440046); // matte directional 60 45
  contrastNormalizedMaterials[MATERIALS.MATTE][45][75] = CustomContrastUniform(gammaFactor,
    meanTarget, 134.60153339832854, targetStd, 27.278002227705667); // matte directional 75 45
  contrastNormalizedMaterials[MATERIALS.MATTE][45][90] = CustomContrastUniform(gammaFactor,
    meanTarget, 104.8302295939948, targetStd, 36.12854919358175); // matte directional 90 45
  contrastNormalizedMaterials[MATERIALS.MATTE][45][100] = CustomContrastUniform(gammaFactor,
    meanTarget, 82.12687053802051, targetStd, 40.47684195258709); // matte directional 100 45

  contrastNormalizedMaterials[MATERIALS.MATTE][45][LIGHTS.MATLAB] = CustomContrastUniform(
    gammaFactor, meanTarget, 145.24504151641847, targetStd, 68.19507984420792); // matte matlab 45

  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][30] = CustomContrastUniform(gammaFactor,
    meanTarget, 80.07235680735263, targetStd, 20.569933157284137); // glossy directional 30 45
  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][45] = CustomContrastUniform(gammaFactor,
    meanTarget, 83.68543601651187, targetStd, 19.997361940379772); // glossy directional 45 45
  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][60] = CustomContrastUniform(gammaFactor,
    meanTarget, 80.15986836188678, targetStd, 19.093637881040486); // glossy directional 60 45
  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][75] = CustomContrastUniform(gammaFactor,
    meanTarget, 70.53981766860578, targetStd, 21.5397875591238); // glossy directional 75 45
  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][90] = CustomContrastUniform(gammaFactor,
    meanTarget, 56.4184812174614, targetStd, 25.029834592078473); // glossy directional 90 45
  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][100] = CustomContrastUniform(gammaFactor,
    meanTarget, 43.447956597920445, targetStd, 26.144498613697245); // glossy directional 100 45

  contrastNormalizedMaterials[MATERIALS.GLOSSY][45][LIGHTS.MATLAB] = CustomContrastUniform(
    gammaFactor, meanTarget, 58.20756089925286, targetStd, 27.61137254246659); // matlab glossy 45

  contrastNormalizedMaterials[MATERIALS.MATTE][60][90] = CustomContrastUniform(gammaFactor,
    meanTarget, 146.0033590060164, targetStd, 15.576613308598121); // matte directional 90 60
  contrastNormalizedMaterials[MATERIALS.MATTE][60][100] = CustomContrastUniform(gammaFactor,
    meanTarget, 128.80968662312245, targetStd, 18.215480673699382); // matte directional 100 60
  contrastNormalizedMaterials[MATERIALS.MATTE][60][110] = CustomContrastUniform(gammaFactor,
    meanTarget, 107.72125271939024, targetStd, 22.50830654458899); // matte directional 110 60
  contrastNormalizedMaterials[MATERIALS.MATTE][60][120] = CustomContrastUniform(gammaFactor,
    meanTarget, 88.87329198357965, targetStd, 23.551441632503828); // matte directional 120 60
  contrastNormalizedMaterials[MATERIALS.MATTE][60][130] = CustomContrastUniform(gammaFactor,
    meanTarget, 72.3866582420558, targetStd, 26.407158004661255); // matte directional 130 60

  contrastNormalizedMaterials[MATERIALS.MATTE][60][LIGHTS.MATLAB] = CustomContrastUniform(
    gammaFactor, meanTarget, 94.97364785265025, targetStd, 37.38996556166459); // matte matlab 60
  ...
  contrastNormalizedMaterials[MATERIALS.GLOSSY][60][LIGHTS.MATLAB] = CustomContrastUniform(
    gammaFactor, meanTarget, 42.512636280303475, targetStd, 16.567008149745796); // matlab glossy 60

  return contrastNormalizedMaterials;
}
...
function RenderImage(data, isPretest = true, normalizeContrast = false) {
  ...
  if (!normalizeContrast) {
    ...
    setMeshMaterial(data.contrastMaterialLookup[data.material][data.light]);
  } else {
